...
exports.insertAVCallMessage = functions.https.onCall(async (data, context) => {
  const { messageID, message, channelID } = data
  ...
  if (messageID) {
      ...
      if (messageDoc.exists) {
        ...
        const newMissedCallUserIDs = [...new Set([...messageData?.missedCallUserIDs, ...message])]
        messageDoc.ref.set({missedCallUserIDs: newMissedCallUserIDs}, { merge: true })
        const data2 = {
          id: channelID,
          content: `You missed a ${messageData?.callType} call`,
          markedAsRead: false,
          createdAt: Math.floor(new Date().getTime() / 1000),
        }
        const promises = message.filter(userId => !newMissedCallUserIDs.includes(userId))
          .map(async(userID) => {
            if (userID) {
              await add(socialFeedsRef.doc(userID), 'chat_feed', data2, true)
              return true
            }
          })
    ...
    const messageData = {
      ...message,
      id: newMessageID,
      missedCallMessage: true, // Flag to differentiate between normal message and missed call message
      createdAt: Math.floor(new Date().getTime() / 1000),
    }
    ...
    const data2 = {
      id: channelID,
      content: `You missed a ${message?.callType} call`,
      markedAsRead: false,
      createdAt: Math.floor(new Date().getTime() / 1000),
    }
    const promises =  message?.missedCallUserIDs.map(async(userID) => {
      if (userID) {
        await add(socialFeedsRef.doc(userID), 'chat_feed', data2, true)
        return true
      }
